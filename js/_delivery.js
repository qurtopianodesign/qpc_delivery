        ...
        $(document).on("click change paste", ".delivery-add-to-cart", function (event) {
            ...
            if (qty !== 0) {
                ...
                $.post(delivery.ajax, {
                    ...
                    // console.log("addClass delivery-add-to-cart");
                    $("#loader").addClass("hidden");
                });
            }
        });
...
// Inizializzazione Barba Js
$().ready(function () {
    Barba.Pjax.start();
    Barba.Prefetch.init(); //Abilita il prefetch
});
        ...
        $.each(obj, function (i, val) {
            ...
            if ($("#row-" + val.id).length === 0) {
                ...
                html =
                    ...
                    "<div class='col-6'>" +
                    "<a href=''>" + rowProduct + "</a>" +
                    "<a href='#' class='no-barba delivery-cart-button delivery-remove-from-cart' delivery-productId='" + val.id + "' ><span class='icon-qpc-delivery-06'></span></a>" +
                    "</div>" +
                    "<div class='col-3'>" +
                //    val.quantity +
                    "<a href='#' class='no-barba delivery-cart-button delivery-add-to-cart' delivery-attributeValue='"+attributeValue+"' delivery-attributeId='"+attributeTheId+"' delivery-attributeTheId='"+attributeTheId+"' delivery-price='" + val.price + "' delivery-productId='" + val.productId + "' delivery-qty='-1' >-</a>" +
                    "<input type='text' class='delivery-add-to-cart' delivery-price='" + val.price + "' delivery-productId='" + val.productId + "'  delivery-qty='" + val.quantity + "' value='" + val.quantity + "' delivery-attributeValue='"+attributeValue+"' delivery-attributeId='"+attributeTheId+"' delivery-attributeTheId='"+attributeTheId+"' />" +
                    "<a href='#' class='no-barba delivery-cart-button delivery-add-to-cart' delivery-attributeValue='"+attributeValue+"' delivery-attributeId='"+attributeTheId+"' delivery-attributeTheId='"+attributeTheId+"' delivery-price='" + val.price + "' delivery-productId='" + val.productId + "' delivery-qty='1' >+</a>" +
                    "</div>" +
