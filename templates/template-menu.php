...
?>
	...
	if ( isset( $wp_query->query_vars['product_slug'] ) ) :
		...
		$product = $QPC_Delivery->getProduct( $product_slug );
		//$product = $QPC_Delivery->getProduct( $category_slug, $product_slug );
		?>
			...
			foreach ( $products as $product ):?>
                ...
                <div class="qpc-typo-menu__item">
                    <a class="qpc-typo-menu__item-link mos<?= $count++ ?>"
                       href="<?= $QPC_Delivery->getURL( $product ); ?>"><span><?= $product["name"]; ?></span></a>
                    <?php if ($image = $QPC_Delivery->getImageForMenuListing($product)) :?>
                    <img class="qpc-typo-menu__item-img" src="<?=$image ?>" alt="<?= $product['name']; ?>"/>
                    <?php endif; ?>
                    <div class="rolling">
