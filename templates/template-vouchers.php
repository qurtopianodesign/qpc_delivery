...
<section class="m-t-10 trigger-header" data-scroll-section id="intro">
	...
	<div class="" data-scroll data-scroll-repeat data-scroll-call="stickyHeader">
		...
		<div data-scroll data-scroll-speed="4" class=" accordion-head-wrap">
			<input type="hidden" name="delivery_lang" id="delivery_lang" value="<?= apply_filters('wpml_current_language', NULL); ?>" />
			<?php
			...
			?>
				<div class=" voucher wpcf7 accordion-head">
					<div class="wrap-qpc-price">
						<span class="qpc-price"><?= QPC_Delivery::getProductName($product); //$name; 
												?></span>
						<div class="wrap-qpc-price-sub">
							<p class="qpc-price"><?= str_replace(",00", "", number_format($product["price"], 2, ",", "")); ?> €</p>
							<button class="delivery-add-to-cart btn icon-qpc-delivery-15" delivery-price="<?= $product['price']; ?>" delivery-productId="<?= $product['id']; ?>" delivery-qty="1"><?php esc_html_e("Aggiungi al carrello", 'mos-theme'); ?>
							</button>
						</div>
					</div>
					<?php
					...
					foreach ($productDetails['attributes'] as $attribute) : ?>
						<div class="wrap-qpc-price">
							<span class="qpc-price"><?= QPC_Delivery::getProductName($product) ?> + <?= QPC_Delivery::getAttributeTitle($attribute); //$title; 
																									?></span>
							<div class="wrap-qpc-price-sub">
								<p class="qpc-price"><?= str_replace(",00", "", number_format($product['price'] + $attribute["price"], 2, ",", "")); ?>
									€
								</p>
								<button class="delivery-add-to-cart btn icon-qpc-delivery-15" delivery-productId="<?= $product['id']; ?>" delivery-price="<?= $product['price']; ?>" delivery-attributeId="<?= $attribute["id"]; ?>" delivery-attributeTheId="<?= $attribute["attribute_id"]; ?>" delivery-attributePrice="<?= $attribute["price"]; ?>" delivery-attributeValue="<?= QPC_Delivery::getAttributeTitle(str_replace("'", "&#39;", $attribute)); ?>" delivery-qty="1"><?php esc_html_e("Aggiungi al carrello", 'mos-theme'); ?>
								</button>
							</div>
						</div>
					<?php endforeach; ?>
